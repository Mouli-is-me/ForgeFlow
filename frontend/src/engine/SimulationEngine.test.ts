...
describe('Deterministic Simulation Engine', () => {
  const baseConfig: SimulationConfig = {
    ...
    stages: [
      { id: 's1', name: 'S1', processingTimeSec: 10, capacity: 100, availability: 100, failureProbability: 0, meanRecoveryTimeSec: 0, machineCount: 1, queueCapacity: 100 },
      { id: 's2', name: 'S2', processingTimeSec: 15, capacity: 100, availability: 100, failureProbability: 0, meanRecoveryTimeSec: 0, machineCount: 1, queueCapacity: 10 }
    ]
  };
  ...
  test('Same seed produces identical results', () => {
    ...
    expect(state1.machines['s1'].processedCount).toBe(state2.machines['s1'].processedCount);
  });

  test('Different seed produces different breakdown events (if non-zero failure)', () => {
    const failConfig = JSON.parse(JSON.stringify(baseConfig)) as SimulationConfig;
